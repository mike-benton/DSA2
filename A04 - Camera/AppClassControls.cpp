...
{
	vector3 targetOffset = m_pCamera->GetTarget() - m_pCamera->GetPosition();
	...
	case sf::Keyboard::Space:
		break;
	case sf::Keyboard::W:
		movement = vector3(targetOffset.x / targetOffset.length() * .5f, 0, targetOffset.z / targetOffset.length() * .5f); //MOVEMENT CONTROLS calculates vector to target
		m_pCamera->SetPosition(m_pCamera->GetPosition() + movement); //adds it to position
		m_pCamera->SetTarget(m_pCamera->GetTarget() + movement); //adds it to target as well so we're still looking in the same direction
		break; 
	case sf::Keyboard::A:
		movement = vector3(targetOffset.z / targetOffset.length() * .5f, 0, targetOffset.x / targetOffset.length() * .5f); //takes orthogonal vector and adds it
		m_pCamera->SetPosition(m_pCamera->GetPosition() + movement);
		m_pCamera->SetTarget(m_pCamera->GetTarget() + movement);
		break;
	case sf::Keyboard::S:
		movement = vector3(targetOffset.x / targetOffset.length() * .5f, 0, targetOffset.z / targetOffset.length() * .5f); //calculates vector to target and then subtracts it
		m_pCamera->SetPosition(m_pCamera->GetPosition() - movement);
		m_pCamera->SetTarget(m_pCamera->GetTarget() - movement);
		break;
	case sf::Keyboard::D:
		movement = vector3(targetOffset.z / targetOffset.length() * .5f, 0, targetOffset.x / targetOffset.length() * .5f); //takes orthogonal vector and subtracts it
		m_pCamera->SetPosition(m_pCamera->GetPosition() - movement);
		m_pCamera->SetTarget(m_pCamera->GetTarget() - movement);
		break;
	}
...
void Application::CameraRotation(float a_fSpeed)
{
	...
	vector3 targetOffset = m_pCamera->GetTarget() - m_pCamera->GetPosition(); //vector from position to target

	
	vector3 deltaX = vector3(targetOffset.z, 0, targetOffset.x); //x and z component of previous vector
	deltaX = vector3(deltaX.x / targetOffset.length(), 0, deltaX.z / targetOffset.length()) * fAngleY; //normalizes vector, then multiplies by the mouse offset
	deltaX += targetOffset;
	deltaX = vector3(deltaX.x / targetOffset.length(), 0, deltaX.z / targetOffset.length());
	deltaX *= targetOffset.length();
	
	/*
	BUG/ERROR:  Logic is flawed here - because I'm always getting a vector that is the sum of the original offset vector and an orthagonal one, I can't ever look behind me
	Also - I wasn't able to make sure the camera stays level - not sure how to do that.
	*/


	vector3 deltaY = vector3(0.0f, targetOffset.y -1, 0.0f); //does basically the same thing - just to y vector
	deltaY = vector3(0.0f, deltaY.y / targetOffset.length(), 0.0f) * fAngleX;
	deltaY += targetOffset;
	deltaY = vector3(0.0f, deltaY.y / targetOffset.length(), 0.0f);
	deltaY *= targetOffset.length(); 




	m_pCamera->SetTarget(m_pCamera->GetPosition() + deltaX + deltaY); //adds vectors to the target's position
