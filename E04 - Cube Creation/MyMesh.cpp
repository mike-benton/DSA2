...
void MyMesh::GenerateCircle(float a_fRadius, int a_nSubdivisions, vector3 a_v3Color)
{
	...
	prevPoint = vector3(0, a_fRadius, 0); //makes sure we're not putting two origin points in the first triangle

	for (int i = 0; i < a_nSubdivisions; i++) {
		currentPoint = vector3((a_fRadius * sin((i + 1) * (2 * PI) / a_nSubdivisions)), (a_fRadius * cos((i + 1) * (2 * PI) / a_nSubdivisions)), 0); //calculates the next position
		AddVertexPosition(currentPoint);

		AddVertexPosition(prevPoint);
		prevPoint = currentPoint; //remembers the last position

		AddVertexPosition(vector3()); //center point
	}
