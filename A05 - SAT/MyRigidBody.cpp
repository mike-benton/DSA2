...
{
	//Some sides are more accurate than others - unsure of reason why
	...
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			R[i][j] = dot(m_m4ToWorld[i], a_pOther->m_m4ToWorld[i]); //fills matrix with values
		}
	}
	
	vector3 t = a_pOther->GetCenterGlobal() - GetCenterGlobal(); //translation vector
	t = vector3(dot(vector4(t, 1), m_m4ToWorld[0]), dot(vector4(t, 1), m_m4ToWorld[1]), dot(vector4(t, 1), m_m4ToWorld[2]));

	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++) {
			AbsR[i][j] = abs(R[i][j]) + FLT_EPSILON; //fills absolute value matrix
		}
	}
